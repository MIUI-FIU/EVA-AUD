...
const DialogueInputComponent = ({ initialDialogue = [], placeholder = "Type your response here..." }) => {
  ...
  // Function to scroll to bottom - used for user input and submission
  ...
  // Handle input change - scroll when typing
  const handleInputChange = (e) => {
    const newInput = e.target.value;
    setUserInput(newInput);
    
    // Only set typing flag to true if there's actual text
  ...
  // Handle input submission - scroll after adding the new message
  const handleSubmit = (e) => {
    e.preventDefault();
    if (userInput.trim() === '') return;
    ...
    // Add a small delay to ensure the DOM has updated with the new message
    setTimeout(() => {
      scrollToBottom();
    }, 10);
  };
  ...
  useEffect(() => {
    if (textareaRef.current) {
      ...
      // If the textarea grows, make sure to scroll to bottom
      if (textareaRef.current.scrollHeight > 38 && isUserTyping) {
        scrollToBottom();
      }
    }
  }, [userInput, isUserTyping]);
